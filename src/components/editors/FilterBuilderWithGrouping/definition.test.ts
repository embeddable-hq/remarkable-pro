...
import type { FilterBuilderGroupingState } from './FilterBuilderWithGrouping.utils';
...
describe('filterBuilderWithGrouping definition', () => {
  it('registers the correct meta', () => {
    expect(filterBuilderWithGrouping.meta.name).toBe('FilterBuilderWithGrouping');
    expect(filterBuilderWithGrouping.meta.label).toBe('Filter Builder (with grouping)');
    expect(filterBuilderWithGrouping.meta.category).toBe('Filters');
  });
  ...
  it('builds filterResults for complete dimension leaf filters (incl. nested groups)', () => {
    ...
    const result = filterBuilderWithGrouping.config.props(inputs as never, [
      state,
      setState,
  ...
  it('skips filters without an operator', () => {
    ...
    const result = filterBuilderWithGrouping.config.props(
  ...
  it('onChange event wraps the clause, falling back to noFilter', () => {
    ...
    expect(filterBuilderWithGrouping.config.events.onChange(clause)).toEqual({ value: clause });
    expect(filterBuilderWithGrouping.config.events.onChange(null)).toEqual({
